...
function FrameRoutineEnd(snapshot) {
    return async function () {
      ...
      if (routineForceEnded) {
          routineTimer.reset();} else if (FrameMaxDurationReached) {
...
function ITIRoutineEachFrame() {
    return async function () {
      ...
      if (t >= 0.0 && itiKey.status === PsychoJS.Status.NOT_STARTED) {
        ...
        // keyboard checking is just starting
        psychoJS.window.callOnFlip(function() { itiKey.clock.reset(); });  // t=0 on next screen flip
        psychoJS.window.callOnFlip(function() { itiKey.start(); }); // start on screen flip
